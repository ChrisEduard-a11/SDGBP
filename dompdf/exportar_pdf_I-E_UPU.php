...
// Consulta: SOLO pagos del UPU logueado y en el rango de fechas
$sql = "
SELECT pagos.* 
FROM pagos
INNER JOIN usuario_pagos ON pagos.id = usuario_pagos.pago_id
WHERE usuario_pagos.usuario_id = $usuario_upu
...
// Tabla con bordes visibles y estilo profesional
    ...
    $html .= '<th style="border:1px solid #ccc;">Cliente</th>';
    $html .= '</tr>';
    $html .= '</thead>';
    $html .= '<tbody>';
...
// Fila de totales
    ...
    // El Saldo Total ahora es la RESTA de los dos anteriores
    $html .= '<td colspan="2" style="border:1px solid #ccc; font-weight:bold;">Saldo Total: Bs ' . number_format($saldo_calculado, 2, ',', '.') . '</td>';
    $html .= '</tr>';
...
// Configurar el tamaño de la página y la orientación (horizontal)
